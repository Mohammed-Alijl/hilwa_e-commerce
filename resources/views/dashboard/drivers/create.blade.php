...
@section('title',__('Front-end/pages/drivers.add.driver'))
@section('css')
...
@section('page-header')
    <div class="row mb-2 mb-xl-3">
        <div class="col-auto d-none d-sm-block">
            <h3><strong>{{__('Front-end/pages/drivers.title')}}</strong> / {{__('Front-end/pages/drivers.add.driver')}}</h3>
        </div>
    </div>
@endsection
@section('content')
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title">{{__('Front-end/pages/drivers.add.driver')}}</h5>
                <h6 class="card-subtitle text-muted">{{__('Front-end/pages/drivers.add.description')}}</h6>
            </div>
            <div class="card-body">
                <form action="{{route('drivers.store')}}" method="post" enctype="multipart/form-data" id="create_user"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function updateZoneOptions(CityId) {
                if (CityId) {
                    $.ajax({
                        url: "{{ URL::to('admin/city-zones') }}/" + CityId,
                        type: "GET",
                        dataType: "json",
                        success: function (data) {
